    ...
    <div className="relative">
      ...
      <aside className={`box-border h-screen transition-all ${expanded ? 'w-5/6 sm:w-64' : 'w-0 sm:w-20'}`}>
        <nav className="flex h-full flex-col border-r bg-white shadow-sm">
          ...
          {/* Aquí he centrado el BranchSelector */}
          ...
          <div className="flex border-t p-3">
            ...
            <div className="flex items-center justify-between transition-all">
              <div className={`leading-4 overflow-hidden transition-all ${expanded ? 'ml-3 max-w-xs' : 'max-w-0'}`}>
                <h4 className="font-semibold">{user?.userUserName ?? 'Usuario'}</h4>
                <span className="text-xs text-gray-600">{user?.userEmail ?? 'correo@desconocido.com'}</span>
              </div>
