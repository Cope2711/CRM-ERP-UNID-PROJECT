...
public interface IGenericServie<T> where T : class
{
    ...
    Task<T?> GetFirstAsync(Expression<Func<T, object>> fieldSelector, object value,
        params Expression<Func<T, object>>[] includes);
    Task<T> GetByIdThrowsNotFoundAsync(Guid id, params Expression<Func<T, object>>[] includes);
    Task<GetAllResponseDto<T>> GetAllAsync(GetAllDto getAllDto, params Expression<Func<T, object>>[] includes);
}
...
public class GenericService<T> : IGenericServie<T> where T : class
{
    ...
    {
        return await _genericRepository.GetByIdAsync(id, includes);
    }
    
    public async Task<T> GetFirstThrowsNotFoundAsync(Expression<Func<T, object>> fieldSelector, object value, params Expression<Func<T, object>>[] includes)
    {
        T? entity = await _genericRepository.GetFirstAsync(fieldSelector, value, includes);

        if (entity == null)
        {
            string fieldName = ((MemberExpression)(fieldSelector.Body is UnaryExpression unary ? unary.Operand : fieldSelector.Body)).Member.Name;
    ...
    public async Task<T?> GetFirstAsync(Expression<Func<T, object>> fieldSelector, object value, params Expression<Func<T, object>>[] includes)
    {
        return await _genericRepository.GetFirstAsync(fieldSelector, value, includes);
    }
    
    public async Task<T> GetByIdThrowsNotFoundAsync(Guid id, params Expression<Func<T, object>>[] includes)
    {
        T? entity = await _genericRepository.GetByIdAsync(id, includes);
    ...
    public async Task<GetAllResponseDto<T>> GetAllAsync(GetAllDto getAllDto, params Expression<Func<T, object>>[] includes)
    {
        ...
        getAllResponseDto.Data = await this._genericRepository.GetAllAsync(getAllDto, includes);
